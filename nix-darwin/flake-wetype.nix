...
let
  ...
  wetypeDeploymentId = "adhoc-installed-v1:${wetypeSource}";
  wetypeRegisteredDeploymentPrefix = "adhoc-registered-manual-enable-v1:${wetypeSource}:";
in
{
    ...
    if wetypeEnable then
      ''
        (
          ...
          wetype_registered_deployment_prefix=${lib.escapeShellArg wetypeRegisteredDeploymentPrefix}
          ...
          if [[ -d "$wetype_target" ]] \
            && [[ "$wetype_existing_deployment_id" == "$wetype_deployment_id" \
              || "$wetype_existing_deployment_id" == "$wetype_registered_deployment_prefix"* ]] \
            && /usr/bin/codesign --verify --deep --strict "$wetype_target" >/dev/null 2>&1; then
            # Migrate the marker written by the previous registrar-based
            # implementation without reinstalling or touching user settings.
            if [[ "$wetype_existing_deployment_id" != "$wetype_deployment_id" ]]; then
              printf '%s\n' "$wetype_deployment_id" >"$wetype_marker"
              /usr/sbin/chown root:wheel "$wetype_marker"
              /bin/chmod 0644 "$wetype_marker"
            fi
